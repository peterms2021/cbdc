...
echo "Skips: Running pip install..."
#pip install -U pip
...
if [ -f "${VERSION_FILE}" ]; then
    ...
    if [ -n "${PYTHON_PACKAGE_PATH}" ]; then
        ...
        pip install -U -e "${PYTHON_PACKAGE_PATH}"
    else
        # Note: Strip unsafe suffix if it exists
        sanitised_version=${VERSION%"+unsafe"}
        echo "Using python package: ${sanitised_version}"
        echo "Skipping CCF package install"    
        #pip install -U ccf=="${sanitised_version}"
    fi
    echo "Skipping requirements.txt install"
    #pip install -U -r "${PATH_HERE}"/requirements.txt
else
    # source tree
    echo "Source tree..."
    BINARY_DIR=.
...
echo "Running ${START_NETWORK_SCRIPT}"
set -x
