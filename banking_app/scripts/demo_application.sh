...
#curl ${server}/app/securities/"$user0_id" -X POST -w "\n%{http_code}" --cacert service_cert.pem --cert user0_cert.pem --key user0_privk.pem -H 'Content-Type: application/json' --data-binary "{ \"userId\": \"$user0_id\", \"holdings\": [{\"securityId\":\"SECURITY1\",\"quantity\":500}]}"
#curl ${server}/app/securities/"$user0_id" -X POST --cacert service_cert.pem --cert user0_cert.pem --key user0_privk.pem -H 'Content-Type: application/json' --data-binary "{ \"userId\": \"$user0_id\", \"holdings\": [{\"securityId\":\"SECURITY2\",\"quantity\":100}]}"

# Request security loan
#curl ${server}/app/loans/"$user1_id" -X POST --cacert service_cert.pem --cert user1_cert.pem --key user1_privk.pem -H 'Content-Type: application/json' --data-binary "{ \"userId\": \"$user1_id\", \"securityId\":\"SECURITY1\",\"quantity\":200}"
