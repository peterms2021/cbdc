...
import {
  ...
  processTransfer,
  pullNewTransaction,
} from "./CcfBridgeServices.js";
import { loadCcfBridgeData, prettyPrint } from "./OpsUtil.js";
...
const logSubscribe = async (): Promise<any> => {
  return new Promise((resolve, reject) => {
    setup().then(([web3, chainId, contractAddress, contract, account]) => {
      ...
      const eventAbis = contract.options.jsonInterface.filter(
...
async function _eventListener() {
  let [pt, et, ea] = await Promise.all([
    pollForTransactions(),
    eventTransferSubScribe(),
    eventApprovalSubScribe(),
  ]);
}

export function bridgeEventListener() {
  loadCcfBridgeData();
  _eventListener().then((res) => {
    console.log(" bridgeEventListener done");
  });
}
