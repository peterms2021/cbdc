...
const CreateHTLCFor: FC<{ cbdc?: Contract }> = (props: { cbdc?: Contract }) => {
  ...
  function handleChangeSender(e: FormEvent, props: FormProps) {
    setSender(props.value);
  }
  ...
  async function handleSubmit() {
    if (props.cbdc) {
      try {
        setLoading(true);
        const response = await props.cbdc.createHTLCFor(
          sender,
          receiver,
          hashlock,
          timelock,
  ...
  return (
    <Form className={styles.form} success error onSubmit={handleSubmit} size="huge">
      <Form.Group inline>
        <label>createHTLCFor</label>
        <Form.Input width={4} placeholder="sender" onChange={handleChangeSender} />
